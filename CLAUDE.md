...
2. Detect workspace root — the **outermost** ancestor containing `.claude/`, **excluding `$HOME`** (subprojects carry their own `.claude/`; `~/.claude` is the user config, not a workspace marker)
